            ...
            if showCandleBar
            {
                ...
                else if open < close
                {
                    ...
                    if dataSet.isIncreasingFilled
                    {
                        context.setFillColor(color.cgColor)
//                        context.fill(_bodyRect)
                        let cornerRadius = CGSize(width: _bodyRect.width / 4.0, height: _bodyRect.width / 4.0)
                        let bezierPath = UIBezierPath(roundedRect: _bodyRect, byRoundingCorners: .allCorners, cornerRadii: cornerRadius)
